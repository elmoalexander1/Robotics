...
class StateAndSafety():
  ...
  def Toggle_cageFunction(self, request):
    ...
    return toggle_cageResponse(self.success)
  ...
  def processVerifying(self):
    if cage == True:
      if (abs(self.goal_cmd.x) > self.cage_x[1] or abs(self.goal_cmd.y) > self.cage_y[1] or self.goal_cmd.z > self.cage_z[1] or self.goal_cmd.z < 0):
  ...
  # This function is called when we are in the moving statec
  ...
  def mainloop(self):
    ...
    while not rospy.is_shutdown():
      ...
      # check if it's verifying, if not, intiate verifying
